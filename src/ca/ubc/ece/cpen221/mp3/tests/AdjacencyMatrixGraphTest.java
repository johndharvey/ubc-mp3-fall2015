...
public class AdjacencyMatrixGraphTest {
    ...
    /**
     * Test method for
     * {@link ca.ubc.ece.cpen221.mp3.graph.AdjacencyListGraph#addVertex(ca.ubc.ece.cpen221.mp3.staff.Vertex)}
     * .
     */
    @Test
    ...
    /**
     * Test method for
     * {@link ca.ubc.ece.cpen221.mp3.graph.AdjacencyListGraph#addEdge(ca.ubc.ece.cpen221.mp3.staff.Vertex, ca.ubc.ece.cpen221.mp3.staff.Vertex)}
     * .
     */
    @Test
    public void testEdges() {
        ...
        assertEquals("The lists should be the same; empty.", EmptyList,
                TestList1);
        assertEquals("The lists should be the same; empty.", EmptyList,
                TestList2);

        assertEquals("v1 has no upstream neighbours", EmptyList,
                TestGraph.getUpstreamNeighbors(testVertex1));
        assertEquals("v1 has no downstream neighbours", EmptyList,
                TestGraph.getDownstreamNeighbors(testVertex1));

        TestGraph.addVertex(testVertex2);

        TestGraph.addEdge(testVertex1, testVertex2);
        assertEquals("There should be an edge from 1 to 2.", true,
                TestGraph.edgeExists(testVertex1, testVertex2));
        assertEquals("There should be no edge from 2 to 1.", false,
                TestGraph.edgeExists(testVertex2, testVertex1));
        ...
        assertEquals("v2 is a downstream neighbour of v1", TestList3,
                TestGraph.getDownstreamNeighbors(testVertex1));
        assertEquals("v1 is an upstream neighbour of v2", TestList4,
                TestGraph.getUpstreamNeighbors(testVertex2));
        assertEquals("v1 has no upstream neighbours", EmptyList,
                TestGraph.getUpstreamNeighbors(testVertex1));
        assertEquals("v2 has no downstream neighbours", EmptyList,
                TestGraph.getDownstreamNeighbors(testVertex2));

        TestGraph.addEdge(testVertex2, testVertex1);

        assertEquals("There should be an edge from 2 to 1.", true,
                TestGraph.edgeExists(testVertex2, testVertex1));

        assertEquals("v1 is a downstream neighbour of v2", TestList4,
                TestGraph.getDownstreamNeighbors(testVertex2));
        assertEquals("v2 is an downstream neighbour of v1", TestList3,
                TestGraph.getUpstreamNeighbors(testVertex1));
    }
